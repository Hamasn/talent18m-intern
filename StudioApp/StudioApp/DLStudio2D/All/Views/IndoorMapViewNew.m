...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        UIImage *floorImage = [UIImage imageWithContentsOfFile:[[NSBundle bundleWithPath:bundlePath] pathForResource:@"studio" ofType:@"png"]];
        _mapView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0,MRScreenHeight/2855*2000, MRScreenHeight)];
...
- (NSMutableArray *)fetchPathPoint:(NSString *)filePath
{
    ...
    NSString *bundlePath = [[NSBundle mainBundle] pathForResource:@"DLStudio2D" ofType:@"bundle"];
    NSString *path = [[NSBundle bundleWithPath:bundlePath]  pathForResource:@"map1_path_data" ofType:@"txt"];
    
    NSFileHandle *fileHandle = [NSFileHandle fileHandleForReadingAtPath:path];
    
    NSData *lineData;
    
    while ((lineData = [self readLineWithDelimiter:@"\n" andFileHandle:fileHandle]))
    {
...
- (NSMutableArray *)fetchPathPairPoint:(NSString *)filePath
{
    NSMutableArray *results = [[NSMutableArray alloc] init];
    NSString *bundlePath = [[NSBundle mainBundle] pathForResource:@"DLStudio2D" ofType:@"bundle"];
    NSString *path = [[NSBundle bundleWithPath:bundlePath]  pathForResource:filePath ofType:@"txt"];
    NSFileHandle *fileHandle = [NSFileHandle  fileHandleForReadingAtPath:path];
    NSData *lineData;
...
- (NSData *)readLineWithDelimiter:(NSString *)theDelimiter andFileHandle:(NSFileHandle*)fileHandle
{
    ...
    unsigned long long currentPosition = [fileHandle offsetInFile];
    ...
    while (lineBreakFound == NO && hasData == YES)
    {
        ...
        lineData = [fileHandle readDataOfLength:bufferSize];
    ...
    [fileHandle seekToFileOffset:currentPosition];
    
    NSData *returnData = [fileHandle readDataOfLength:positionOffset];
