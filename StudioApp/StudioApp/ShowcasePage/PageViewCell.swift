...
class PageViewCell: UITableViewCell{
    ...
    override func awakeFromNib() {
        ...
        var inset = PageControl.frame.height / 2
...
extension PageViewCell : FSPagerViewDataSource,FSPagerViewDelegate {
    ...
    //toppicture
        ...
        TopPicture.snp.makeConstraints { (make) -> Void in
            ...
            make.bottom.equalTo(cell).offset(-494)
            make.right.equalTo(cell).offset(0)
        }
        TopPicture.backgroundColor = UIColor.white
        //set up gradient layer for top picture
        let gradientLayer = CAGradientLayer()
        gradientLayer.frame = TopPicture.layer.bounds;
        TopPicture.layer.addSublayer(gradientLayer)
        gradientLayer.startPoint = CGPoint(x: 0.0, y: 0.0)
        gradientLayer.endPoint = CGPoint(x: 1.0, y: 0.0)
         gradientLayer.colors = [UIColor.init(red: 247.0/255.0, green: 86.0/255.0, blue: 218.0/255.0, alpha: 0.86).cgColor,UIColor.init(red: 14.0/255.0, green: 105.0/255.0, blue: 238.0/255.0, alpha: 0.86).cgColor]
        
    //main picture
        let MainPicture = UIImageView()
        cell.addSubview(MainPicture)
        MainPicture.snp.makeConstraints { (make) -> Void in
            make.top.equalTo(TopPicture.snp.bottom).offset(0)
            make.left.equalTo(cell).offset(0)
